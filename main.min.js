var b=document.getElementById("canvas"),c=b.getContext("2d",{alpha:!1}),e=b.width,aa=b.height,ba,ca=-1,da=-1,h=0,k=0,l=0,m=0,p=1E3,q=0,r=!1,t=0,u=3,ea=.5,v=!1,w=0,fa=!1,x=0,y=0,z=0,ha=0,A=!1,B=0,C=0,D=!1,E=0,F=0,G=0,H=.7,I=0,J=0,K=!1,ia=0,ja=0,L=!0,M=!1,ka=[],N=[],O=parseInt(localStorage.getItem("ohflip.goalIdx"))||0;N.push({text:"Haz un giro",b:P,a:1});N.push({text:"Aterriza dos giros x1 seguidos",b:Q,a:2});N.push({text:"Aterriza perfectamente",b:la,a:1});N.push({text:"Aterriza un giro de 20 m",b:R,a:20});
N.push({text:"Haz un giro",b:P,a:2});N.push({text:"Aterriza tres giros seguidos",b:Q,a:3});N.push({text:"Aterriza sobre tu cabeza",b:ma,a:1});N.push({text:"Haz un giro x3",b:P,a:3});N.push({text:"Aterriza perfectamente dos veces seguidas",b:na,a:2});N.push({text:"Alcanza la altura de 50 m",b:R,a:5});N.push({text:"Aterriza cuatro giros seguidos",b:Q,a:4});N.push({text:"Haz un giro x4",b:P,a:4});N.push({text:"Aterriza 5 giros seguidos",b:Q,a:5});N.push({text:"Aterriza perfectamente 3 veces seguidas",b:na,a:3});
N.push({text:"Aterriza un giro de 100 m",b:R,a:10});N.push({text:"Haz un giro x5",b:P,a:5});N.push({text:"Aterriza 10 giros seguidos",b:Q,a:10});N.push({text:"Alcanza la altura de 250 m",b:R,a:250});N.push({text:"Aterriza 5 giros seguidos",b:na,a:5});N.push({text:"Haz un giro x10",b:P,a:7});N.push({text:"Alcanza la altura de 500 m",b:R,a:500});var S=0;document.addEventListener("mousedown",function(a){K=!0;T(a)},!1);document.addEventListener("mouseup",function(a){K=!1;T(a)},!1);
...
90)/360),y>a&&(U(.5*b.width+100,b.height-200,"x"+y,"#D37CFF"),0<l&&ha++),180<=m?m-=360:-180>m&&(m+=360),l+=-1400*d,k+=l*d,E=Math.max(k,E),0>=k&&(F=16,G=0,30<Math.abs(m)?(v=!0,w=1,fa=.5>Math.random(),U(.5*b.width+100,b.height-100,"Fallaste","#F42"),145<Math.abs(m)&&(D=!0)):(a=270<=t,A=6.5>Math.abs(m),p=a?p+(A?180:120)*(1+y/5*.5):Math.max(p-120,1E3),a&&A&&!L&&(J=.025),a?(z=y,x+=y,B++,A&&C++,A?U(.5*b.width+100,b.height-100,"perfecto!","#FF0"):U(.5*b.width+100,b.height-100,"Genial","#0F4")):C=B=0),O<N.length&&
N[O].b(N[O])&&(U(b.width-100,120,"Completado!","#FF0",!0),S=1,C=B=0),k=0,l=p,r=!0,ha=y=z=t=0,D=!1,E=0),u-=d,ea-=d,0>=u&&(u=1+3*Math.random(),ea=.1+.1*Math.random()));a=280/Math.max(k,280)*1.5;I=a<H?3:I-d;a=Math.min(H,a);.5>a&&(a=Math.pow(a,.97));H+=.2*(a-H);0>=I&&(H+=.001*(.7-H));J*=.8;F*=.9;G+=4E3*d;c.save();c.setTransform(1,0,0,1,0,0);c.fillStyle="#AADDFF";c.fillRect(0,0,b.width,b.height);c.restore();c.save();c.scale(H+J,H+J);e=b.width/(H+J);aa=b.height/(H+J);c.translate(.5*(e-b.width),aa-b.height);
...
function sa(){c.save();c.setTransform(1,0,0,1,0,0);if(L)Z("oh, flip!",.5*b.width,160,-5*Math.PI/180,170,"center","#000"),Z("oh, flip!",.5*b.width-10,155,-5*Math.PI/180,170,"center","#FF9600"),Z("Un juego de giros en el aire",.5*b.width,240,-5*Math.PI/180,50,"center","#000"),Z("Un juego de giros en el aire",.5*b.width-4,235,-5*Math.PI/180,50,"center","#FFF"),Z("Aterriza giros en el aire para ganar altura",.5*b.width,b.height-20,0,25,"center","#000"),Z("Aterriza giros en el aire para ganar altura",
.5*b.width-3,b.height-23,0,25,"center","#FFF");else{var a=Math.floor(k/40),d=localStorage.getItem("ohflip.maxHeightFt");if(null===d||a>d)localStorage.setItem("ohflip.maxHeightFt",a),d=a;Z("Altura: "+a+" m (Giro más alto: "+d+" m)",12,27,0,20,"left","#000");a=localStorage.getItem("ohflip.maxTotalFlips");if(null===a||x>a)localStorage.setItem("ohflip.maxTotalFlips",x),a=x;Z("Giros: "+x+" (Cantidad de giros: "+a+")",12,50,0,20,"left","#000");a="#000";0<S&&(a=.075>S%.15?"#000":"#00FF00");O<N.length?(Z("Objetivo #"+(O+1)+":",
b.width-12,27,0,20,"right",a),Z(N[O].text,b.width-12,50,0,20,"right",a)):(a=400>Date.now()%800?"#000":"#FF9600",Z("Felicidades! Completaste todos los objetivos",b.width-12,27,0,20,"right",a),Z("Presiona acá para volver a jugar",b.width-12,50,0,20,"right",a))}ka.forEach(function(f){var g=Math.min(f.time/.1,1),n=Math.min(f.time/.4,1),V=25*Math.sin(n*Math.PI*.5);n=50*Math.sin(n*Math.PI*.5);var W=f.c?20:30,oa=f.c?10:25;Z(f.text,f.x+V,f.y-n,-5*Math.PI/180,W+Math.sin(g*Math.PI*.75)*oa,"center","#000");
